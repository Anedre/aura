...
const fmt = (n: number | null | undefined, d = 2) =>
  (isFiniteNum(Number(n)) ? Number(n).toFixed(d) : "—");
function clsAction(a: Signal["action"]) {
  return a === "BUY"
    ? { stroke: "stroke-emerald-400", fill: "fill-emerald-400" }
    : a === "SELL"
    ? { stroke: "stroke-rose-400", fill: "fill-rose-400" }
    : a === "ABSTAIN"
    ? { stroke: "stroke-amber-400", fill: "fill-amber-400" }
    : { stroke: "stroke-slate-400", fill: "fill-slate-400" };
}
const toSide = (a?: Signal["action"]) =>
  (a === "BUY" ? "BUY" : a === "SELL" ? "SELL" : undefined) as "BUY" | "SELL" | undefined;
...
function signalBands(sig: Signal): { lowF: number; highF: number } | null {
  if (
    isFiniteNum(sig.band_low) &&
    isFiniteNum(sig.band_high) &&
    sig.band_low! > 0 &&
    sig.band_high! > 0
  ) {
...
function LineChart({
  ...
  const path = useMemo(() => {
    ...
    return data
      .map((d, i) => `${i ? "L" : "M"} ${x(i).toFixed(2)} ${y(Number(d.c) || 0).toFixed(2)}`)
      .join(" ");
  }, [data, x, y]);
  ...
  function onClick(e: React.MouseEvent<SVGSVGElement>) {
    const idx = idxFromClientX(e.clientX);
    if (idx == null) return;
    const c = Number(data[idx]?.c);
    if (isFiniteNum(c)) onPickPrice?.(c);
  }
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {signals.map((s, idx) => {
          const i = nearestIndex(s.ts, data);
          const bar = data[i];
          if (!bar) return null;
          const cx = x(i);
          ...
          const color = clsAction(s.action);
          return (
            <g
              key={idx}
      ...
      {hoverIdx != null && data[hoverIdx] && (
        <div className="absolute top-2 left-2 rounded-lg bg-black/80 text-white text-xs px-2 py-1 border border-white/10 pointer-events-none">
          ...
          <div>
            O:{fmt(data[hoverIdx].o, 4)} H:{fmt(data[hoverIdx].h, 4)} L:{fmt(data[hoverIdx].l, 4)} C:
            {fmt(data[hoverIdx].c, 4)}
          </div>
...
function Simulate({
  ...
  externalSide?: "BUY" | "SELL" | undefined;
}) {
  ...
  useEffect(() => {
    try {
      const u = localStorage.getItem("aura:user");
      if (u) setUser(u);
    } catch {}
  }, []);
  useEffect(() => {
    try {
      localStorage.setItem("aura:user", user);
    } catch {}
  }, [user]);
  useEffect(() => {
    if (externalPrice != null) setPrice(externalPrice);
  }, [externalPrice]);
  useEffect(() => {
    if (price == null) setPrice(lastPrice);
  }, [lastPrice, price]);
  ...
  async function doTrade(side: "BUY" | "SELL") {
    ...
    try {
      const payload: PaperOrderRequest = {
        ...
        fees_bp: norm(fees, 10),
        slippage_bp: norm(slip, 5),
      };
      const res: PaperTrade = await postPaperTrade(payload);
      const eff = res.effective_price ?? norm(price, lastPrice);
      setMsg(`OK ${side} @ ${Number(eff).toFixed(4)} (efectivo). Ver en Paper.`);
    } catch (e: unknown) {
    ...
    <section className="rounded-xl border border-white/10 bg-white/[0.02] p-4 space-y-3">
      ...
      {externalSide && (
        <div className="text-xs opacity-80 -mt-1">
          Sugerencia desde Feed: <b>{externalSide}</b> @ {fmt(externalPrice, 4)}
        </div>
      )}
      ...
      {msg && (
        <p className="text-xs opacity-80">
          {msg} <a href="/paper" className="underline">Ir a Paper</a>
        </p>
      )}
    </section>
  );
      ...
      <div className="grid sm:grid-cols-4 gap-3">
        ...
        <div className="rounded-lg bg-white/5 border border-white/10 p-2">
          ...
          <div className="text-base font-semibold">
            {fmt(stops.tp, 4)} <span className="text-xs opacity-70">({fmt(up, 2)}%)</span>
          </div>
        </div>
        <div className="rounded-lg bg-white/5 border border-white/10 p-2">
          <div className="text-[11px] opacity-70">SL</div>
          <div className="text-base font-semibold">
            {fmt(stops.sl, 4)} <span className="text-xs opacity-70">({fmt(dn, 2)}%)</span>
          </div>
        </div>
        <div className="rounded-lg bg-white/5 border border-white/10 p-2">
          ...
          <div
            className={`text-base font-semibold ${
              rr != null ? (rr >= 2 ? "text-emerald-300" : rr >= 1 ? "text-amber-300" : "text-rose-300") : ""
            }`}
          >
...
export default function AssetDetailPage() {
  ...
  // recomendación por activo (single)
  ...
  const initialPickedSide = useMemo(() => {
    ...
    return s === "BUY" || s === "SELL" ? (s as "BUY" | "SELL") : undefined;
  }, [search]);
  const initialPickedPrice = useMemo(() => {
    const p = Number(search.get("price"));
  ...
  useEffect(() => {
    setPickedSide(initialPickedSide);
  }, [initialPickedSide]);
  ...
  async function askRecommendation() {
    ...
    try {
      ...
      const one: AssetRecommendation | null = Array.isArray(r) ? r[0] ?? null : r;
      setRec(one);
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : String(e);
      setRecErr(msg);
    ...
    <main className="min-h-dvh bg-background text-foreground">
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div className="space-y-0.5">
            <h1 className="text-2xl font-bold">
              {(data.symbol || symbol).toUpperCase()} — Detalle
            </h1>
            <div className="flex items-center gap-2 text-xs opacity-70">
              {data.horizon && (
                <span className="px-2 py-0.5 rounded-full ring-1 ring-white/15 bg-white/5">
                  Ritmo: {data.horizon}
                </span>
              )}
              <span className="px-2 py-0.5 rounded-full ring-1 ring-white/15 bg-white/5">
            ...
            <div className="flex rounded-lg border border-white/10 overflow-hidden">
              {[30, 60, 120].map((n) => (
                <button
                  key={n}
                  onClick={() => setBars(n)}
                  className={`px-2 py-1 text-xs border-r border-white/10 last:border-r-0 ${
                    bars === n ? "bg-white/15" : "bg-white/5 hover:bg-white/10"
                  }`}
                >
                  {n}
                </button>
        ...
        <section className="rounded-xl border border-white/10 bg-white/[0.02] p-4 space-y-3">
          ...
          {rec && (
            <div className="grid sm:grid-cols-4 gap-3 text-sm">
              ...
              <div className="rounded-lg bg-white/5 border border-white/10 p-2">
                ...
                <div className="text-base font-semibold">
                  {rec.p_conf != null ? `${Math.round(rec.p_conf * 100)}%` : "—"}
                </div>
              </div>
              <div className="rounded-lg bg-white/5 border border-white/10 p-2">
                <div className="text-[11px] opacity-70">TP</div>
              ...
              <div className="sm:col-span-4">
                <button
                  onClick={() => {
                    setPickedSide(
                      rec.action === "BUY" ? "BUY" : rec.action === "SELL" ? "SELL" : undefined,
                    );
                    setPickedPrice(last);
                  }}
          ...
          {!rec && !recLoading && (
            <p className="text-xs opacity-70">
              Pide una recomendación para este símbolo. Si el backend no tiene endpoint, se usa un
              fallback desde el feed 1d.
            </p>
          )}
        </section>
        ...
        <section className="rounded-xl border border-white/10 bg-white/[0.02] p-4 space-y-2">
          ...
          {sigs.length > 0 ? (
            <p className="text-xs opacity-70">
              Clic en una señal para usar su precio/side en el simulador.
            </p>
          ) : (
        ...
        <Simulate
          ...
          lastPrice={last}
          externalPrice={pickedPrice}
          externalSide={pickedSide}
        />
      </div>
    </main>
  );
